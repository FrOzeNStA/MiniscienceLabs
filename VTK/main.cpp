...
public:
    ...
    void move(double tau, double currentTime) {
        x += vx * tau;
        y += vy * tau;
        z += vz * tau;

        // Twisting, full size of the model is 236 and 110 is the starting level of twisting part
        double centerX = 110.0;
        double centerY = 110.0;
        
        double twistAmount = sin(currentTime * 1.5) * 0.3;
        
        double twistFactor = (z - (-0.6)) / 236.0;
        
        double angle = twistAmount * twistFactor;
        double x_rel = x - centerX;
        double y_rel = y - centerY;

        x = centerX + x_rel * cos(angle) - y_rel * sin(angle);
        y = centerY + x_rel * sin(angle) + y_rel * cos(angle);
...
public:
    ...
    CalcMesh(const std::vector<double>& nodesCoords, const std::vector<std::size_t>& tetrsPoints) {
        ...
        for(unsigned int i = 0; i < nodesCoords.size() / 3; i++) {
            ...
            double r = sqrt(pointX*pointX + pointY*pointY + pointZ*pointZ);
            double smth = AMPLITUDE * cos(-K * r);
    ...
    void snapshot(unsigned int snap_number) {
        ...
        string fileName = "hand-step-" + std::to_string(snap_number) + ".vtu";
